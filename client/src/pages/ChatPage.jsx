...
export default function ChatPage() {
    ...
    return (
        <div className="chat-page-container flex flex-col bg-zinc-900/50 md:rounded-2xl border-t md:border border-zinc-800 overflow-hidden relative md:m-6 shadow-2xl">
            <style>{`
                .chat-page-container {
                    position: fixed;
                    inset: 0;
                    top: var(--header-h, 57px);
                    bottom: calc(var(--bottom-nav-h, 4rem) + env(safe-area-inset-bottom, 0px));
                    z-index: 0;
                }
            ...
            <div className="flex items-center justify-center gap-2 px-4 py-2.5 border-b border-zinc-800/80 bg-zinc-900/60 backdrop-blur-sm flex-shrink-0">
                <div className="relative">
                    ...
                    >
                        <span className="truncate max-w-[130px] md:max-w-[200px]">{models.find(m => m.id === selectedModel)?.name}</span>
                        ...
                        <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-64 bg-zinc-800 border border-zinc-700 rounded-xl shadow-xl overflow-hidden py-1 animate-in fade-in slide-in-from-top-2 z-50">
                            {models.map(model => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 text-sm transition-colors flex items-center justify-between ${selectedModel === model.id
...

            {
                !id && messages.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center p-5 md:p-8 text-center overflow-y-auto">
                        <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-tr from-indigo-600 to-purple-600 rounded-2xl flex items-center justify-center mb-4 md:mb-6 shadow-lg shadow-indigo-500/20">
                            <Bot className="w-8 h-8 md:w-10 md:h-10 text-white" />
                        </div>
                        <h2 className="text-xl md:text-2xl font-bold text-white mb-2">How can I help you today?</h2>
                        <p className="text-zinc-400 text-sm max-w-md mb-6 md:mb-8">
                    ...
                    <>
                        <div className="flex-1 overflow-y-auto p-3 md:p-6 space-y-4 md:space-y-6 scroll-smooth overscroll-contain" ref={scrollRef}>
