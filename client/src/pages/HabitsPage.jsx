...
export default function HabitsPage() {
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 md:space-y-10 relative mobile-page-pad pb-24 md:pb-8 overflow-x-hidden">
            ...
            {/* Header */}
            <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 md:gap-4 overflow-hidden">
                <div className="w-full md:w-auto">
                    <h1 className="text-3xl md:text-4xl font-black bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent tracking-tighter">Habit Tracker</h1>
                    <div className="flex items-center gap-2 text-xs font-bold text-zinc-500 mt-2 uppercase tracking-widest">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 hide-scrollbar scroll-smooth">
                    <button
                        onClick={handleAnalyze}
                        disabled={isAnalyzing}
                        className="flex-none px-3 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2 disabled:opacity-50 whitespace-nowrap text-[11px] uppercase tracking-wider"
                    >
                    ...
                    <div className="flex-none flex bg-zinc-900/80 backdrop-blur-md border border-white/5 rounded-xl p-1 shadow-xl">
                        <button
                            onClick={() => setViewMode('table')}
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all text-center ${viewMode === 'table' ? 'bg-zinc-700/80 text-white shadow-inner' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            Table
                        </button>
                        <button
                            onClick={() => setViewMode('grid')}
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all text-center ${viewMode === 'grid' ? 'bg-zinc-700/80 text-white shadow-inner' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            Grid
                        </button>
                        <button
                            onClick={() => setViewMode('history')}
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all text-center flex items-center gap-1.5 ${viewMode === 'history' ? 'bg-zinc-700/80 text-white shadow-inner' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {Array.from({ length: 12 }).map((_, monthIndex) => {
                        ...
                        return (
                            <div 
                                key={monthIndex} 
                                className="bg-zinc-900/30 border border-zinc-800/50 p-4 rounded-xl hover:border-indigo-500/50 hover:bg-zinc-800/40 transition-all cursor-pointer group"
                                onClick={() => {
                            ...
                            >
                                <div className="flex items-center justify-between mb-3">
                                    <h3 className="text-sm font-bold text-zinc-300 group-hover:text-white">{monthName}</h3>
                                    <div className="w-2 h-2 rounded-full border border-zinc-700 bg-zinc-800 group-hover:border-indigo-500/50 transition-colors" />
                                </div>

                                <div className="grid grid-cols-7 gap-1">
                                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                                        <div key={i} className="text-[8px] text-zinc-600 text-center font-bold">{d}</div>
                                    ))}
                                    ...
                                    {slots.map((date, i) => {
                                        ...
                                        if (intensity > 0.6) bgClass = "bg-indigo-400/80";
                                        if (intensity === 1) bgClass = "bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.2)]";
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`aspect-square rounded-sm ${bgClass} ${isToday ? 'ring-1 ring-white/50' : ''} transition-all hover:scale-110 flex items-center justify-center`}
                                            >
                                                <span className="text-[6px] text-zinc-400/80 font-medium">{date.getDate()}</span>
                                            </div>
                                        )
                                    })}
                                </div>
