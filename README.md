...
- **Bottom Navigation Bar** — persistent mobile nav with smooth spring-animated active indicators.
...
- Bottom tab navigation with active state animations
...
### April 2026
- 📱 **Mobile UI Overhaul** — comprehensive mobile optimization across all pages:
  ...
  - Bottom nav updated with `safe-area-inset-bottom` padding and fixed active indicator positioning
  - Applied consistent mobile bottom padding to HabitsPage, JournalPage, ProfilePage, GoalsPage, FocusPage
  - Removed hardcoded `pb-16` from Layout main; each page manages its own spacing
  - ChatPage model selector moved from absolute to normal document flow
