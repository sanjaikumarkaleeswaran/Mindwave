...
## 4. Vision-Supported Image Analysis (NEW)

We have now integrated **Multimodal Vision** support directly into the Chat backend. This allows you to upload screenshots (like from Strava or fitness apps) and have the AI "see" and analyze them.

### How it Works:
1. **Dynamic Model Switching**: When you attach an image (`.jpg`, `.png`, etc.), the backend automatically switches from the standard text model to the high-performance **`meta-llama/llama-4-scout-17b-16e-instruct`** vision model.
2. **Base64 Encoding**: The server converts your image file into a Base64 data string, which is then sent securely to Groq's Vision API.
3. **Optimistic Previews**: The frontend uses standard Web APIs (`URL.createObjectURL`) to show you an instant, high-quality preview of the image in your chat bubble before the server even responds.
4. **Interactive Lightbox**: Any image in your chat history can be clicked to open in a full-screen blurred "Lightbox" mode, allowing you to zoom in on statistics or map details in your screenshots.
